...
#[no_mangle]
pub extern fn expf(mut i: f32) -> f32 {
    if i < F32_LOW {
        // e to the power of less than F32_LOW is +0.
        return 0.0;
    } else if i > F32_HIGH {
        // Overflow, NAN of +∞.
        return f32::MAX_VALUE * i;
    }
    ...
    if bits > 0x3EB17218 { // |i| > 0.5ln(2)
        if bits > 0x3F85_1592 { // |i| > 1.5ln(2)
...
/// Calculate e to the power of i.
#[no_mangle]
pub extern fn exp(mut i: f64) -> f64 {
    if i < F64_LOW {
        return 0.0;
    } else if i > F64_HIGH {
        // Overflow, NAN of +∞.
        return f64::MAX_VALUE * i;
    }
    ...
    if bits > 0x3FD6_2E42_FEFA_39EF { // |i| > 0.5ln(2)
        if bits > 0x3FF0_A2B2_3F3B_AB73 { // |i| > 1.5ln(2)
